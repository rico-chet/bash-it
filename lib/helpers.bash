...
# Alias for reloading dotfiles
# shellcheck disable=SC2139
alias reload_dotfiles="$(_make_reload_alias dotfile dotfile)"
...
function _bash-it-migrate() {
	...
	for file_type in "aliases" "dotfiles" "plugins" "completion"; do
...
function _bash-it-profile-save() {
	...
	for subdirectory in "plugins" "dotfiles" "completion" "aliases"; do
...
function _disable-dotfile() {
	_about 'disables bash_it dotfile'
	_param '1: dotfile name'
	_example '$ disable-dotfile emacs'
	_group 'lib'
...
function _disable-dotfile() {
	...
	_example '$ disable-completion emacs'
	_group 'lib'
...
function _enable-dotfiles() {
	...
	_enable-thing "dotfiles" "dotfile" "${1?}" "$BASH_IT_LOAD_PRIORITY_DOTFILE"
}
